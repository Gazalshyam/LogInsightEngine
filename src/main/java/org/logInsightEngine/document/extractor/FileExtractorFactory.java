...
public class FileExtractorFactory {
    ...
    public FileExtractor getExtractor(MultipartFile multipartFile) throws UnsupportedFileTypeException {

        for (FileExtractor extractor : extractors) {
            if (extractor.supports(multipartFile)) {
                return extractor;
            }

        }

        throw new UnsupportedFileTypeException("Unsupported file type: " + multipartFile.getOriginalFilename());
    }
}
